...
const { loadOrCreateProfileMeta } = require('./profile-meta');
const { getStealthScript, CHROME_USER_AGENT } = require('./stealth');
...
function buildPersistentOptions({
	...
	const options = {
		...
		locale: profileMeta.locale || 'ru-RU',
		timezoneId: profileMeta.timezoneId || 'Europe/Moscow',
		viewport: profileMeta.viewport,
		userAgent: profileMeta.userAgent || CHROME_USER_AGENT,
		args: [
	...
	const canMigrate =
		legacyStoragePath && fs.existsSync(legacyStoragePath) && !isProfileInitialized(userDataDir);
...
async function launchPersistentBrowser(config, trySystemChrome) {
	...
	const profileMeta = loadOrCreateProfileMeta(userDataDir);
	...
	try {
		...
		await context.addInitScript({ content: getStealthScript(profileMeta) });

		context._hhLockPath = lockPath;
		context._hhUserDataDir = userDataDir;
		context._hhProfileMeta = profileMeta;
